...
class TestTaskService(unittest.TestCase):
    def setUp(self):
        ...
        self.service = TaskService(
            task_repo=self.mock_repo, workflow_service=self.mock_workflow_service
        )
    ...
    def test_update_assigned_task_only_task_not_found(self):
        ...
        with self.assertRaises(Forbidden):
            self.service.update_assigned_task_only(
                {"username": "test_user"}, "task_id", 2
            )
    ...
    def test_update_assigned_task_only_success(self):
        ...
        result = self.service.update_assigned_task_only(
            {"username": "test_user"}, "task_id", 2
        )
    ...
    @patch("helpers.util.convert_str2time")
    @patch("helpers.util.parse_sort_query")
    def test_view_employee_task_report(
        self, mock_parse_sort_query, mock_convert_str2time
    ):
        mock_convert_str2time.side_effect = lambda x: x
        mock_parse_sort_query.return_value = []
        ...
        result = self.service.view_employee_task_report(
            0, 10, {}, "2023-01-01 00:00:00", "2023-12-31 00:00:00", "-created_at"
        )
